...
# *********** Class : Spring 2019  ***********#
# *********** Part3 : Dijkstra for Rigid Robot  ***********#
...
def create_graph(rrad, clrnc, resol):
    ...
    #points = [x for x in graph.keys() if not (graph[x]['valid'])]
...
def Dijkstra(start, resol_grid, goal, points):
    ...
    source_node = Algorithm_PathCalcNode(start[0], start[1], 0, 0, -1)
    goal_node = Algorithm_PathCalcNode(goal[0], goal[1], 0, 0, 0)
    ...
    while len(data_setopen) != 0:
        ...
        Node_Current_ID = min(data_setopen, key=lambda i: data_setopen[i].cost)
        ...
        if len(points_2plot) % (round(100 / resol_grid)) == 0:
            plt.plot(x, y, "g.")
            plt.grid(True)
            plt.axis("equal")
            plt.grid(b=True, which='major', color='y', linestyle='-')
        ...
        node_setclosed[Node_Current_ID] = Node_Current

        # Creating the new neighbor nodes
        Node_Neighbor = Neighbor_NodeCreate(Node_Current, points, resol_grid)
...
# Defining the Class Algorithm_PathCalcNode and initializing the values in the class
class Algorithm_PathCalcNode:

    def __init__(self, x, y, cost, id, par_id):
        self.x = x
        self.y = y
        self.cost = cost
        self.id = id
...
def Neighbor_NodeCreate(Node_Current, obstacle_points, resol_grid):
    X = Node_Current.x
    Y = Node_Current.y
    ...
    for i in range(0, 8):
        ...
        if Gen_X in range(0, round(251 / resol_grid)) and Gen_Y in range(0, round(151 / resol_grid)):
            if Pos not in obstacle_points:
                ...
                neighbor_node = Algorithm_PathCalcNode(Gen_X, Gen_Y, cost_new, ID, Node_Current.id)
...
if __name__ == "__main__":
    ...
    plt.title("Dijikstra Simulation for Rigid Robot")

    # Checking whether the input node points are in the obstacle space
    input = (sx, sy)  # start node points
    if g[sx, sy]['valid'] == True and g[gx, gy]['valid'] == True and sx <= (round(250 / resol)) and sy <= (
            round(150 / resol)) and gx <= (round(250 / resol)) and gy <= (round(150 / resol)):

        Dijkstra((sx, sy), resol, (gx, gy), points)
